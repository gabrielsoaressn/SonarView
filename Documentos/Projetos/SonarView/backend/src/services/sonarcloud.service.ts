...
export class SonarCloudService {
  private readonly baseUrl = 'https://sonarcloud.io/api';
  private readonly token = process.env.SONARCLOUD_TOKEN || '1d031b3852fefb184a4a79dc6de9c8b96df5b818';
  private readonly projectKey = process.env.PROJECT_KEY || 'gabrielsoaressn_clone-fklearn';
  ...
  async getProjectMeasures(): Promise<SonarCloudMetrics> {
    try {
      const metrics = [
        // Reliability metrics
  ...
  async getProjectHistory(metrics: string[], from?: string) {
    try {
      const response: AxiosResponse = await axios.get(
  ...
  private transformMeasures(data: any): SonarCloudMetrics {
    const measures = {};
  ...
  private calculateOverallRating(measures: any): string {
    const ratings = [
